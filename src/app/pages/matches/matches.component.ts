...
export class MatchesComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private filtercomp: FilterComponent, public route: Router, public snack: SnackService,private routes: ActivatedRoute) {
  ...
  getData() {
    ...
    this.http.get('http://matchmakerz.in/api/v1/client/total-shortlist?id=' + this.routes.snapshot.queryParamMap.get('id'), {
      headers: headers
    }).pipe(debounceTime(1000)).subscribe((res) => {
  ...
  getshortlisted() {
    this.route.navigate(['/shortlisted'],{ queryParams: { id:this.routes.snapshot.queryParamMap.get('id')}});
  }

  filter() {
    this.route.navigate(['/filter'],{ queryParams: { id:this.routes.snapshot.queryParamMap.get('id')}})
  }

  getProfile(data) {
    localStorage.setItem('clientId', data);
    this.route.navigate(['/client-profile'],{ queryParams: { id:data}});
  }
  GetMore() {
    console.log("********")
    this.show = false;
